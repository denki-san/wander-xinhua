...
- Coordinate source: `docs/research/shanghai-orchestra-osm-binding.json` 保存五个候选 way 的原始 WGS84；运行时 frame 继续使用 `[-44.4, 44]`、yaw `2.761592653589793`、scale `0.88`
- Scene position: frame 本身只负责稳定局部坐标；五个候选 footprint 逐点回投 OSM，Recovery 最大误差 `0.000006565` 场景单位
- Confidence: footprint 几何高；五个 way 是否属于上海民族乐团、以及 6/7/8 号楼一一对应关系低
...
| --- | --- | --- | --- | --- |
| Massing | 五个 OSM 候选与 Recovery clean-v2 分体 | Recovery 固定机位保留；MCP1 等待成员闭合 | Recovery runtime pass 保留 | Geometry pass / membership blocked |
| Runtime calibration | WGS84 回投、局部 frame、机位和双道路退界 | N/A | 不覆盖 Recovery；地图终审等待成员闭合 | Geometry pass / formal map blocked |
...
- Changes: 从 Recovery/Hold 选择性保留三张证据、Brief、clean-v2 Blend/GLB 与固定机位图；新增本栋 WGS84 绑定、可复算地图专项和单建筑再生成器。未修改公共 registry、runtime、Fast manifest 或旧 Hero。
- Evidence used: TJAD 三张本地照片；Recovery/Hold 的 2026-07-24 Overpass 原始 way 几何；生产 `xinhua-map-data.json` 道路与投影合同。
- Graybox runtime result: 沿用 Recovery 已通过的 runtime 阶段，未重复采集或覆盖截图。
- Blender result: 未把 Recovery 固定机位图冒充 MCP1；正式 MCP1 状态为 `pending-formal-member-set`。
- GLB result: Recovery GLB 保持 SHA `63eb25ca4abcbab0e434adb528ea0f37650495c614833f7b9bd05afbc519484c`、`9132 bytes`、`5 nodes`、`76 triangles`、`1 material`、`0 images`。本栋再生成器只在 `/tmp/test_...` 输出审计候选，没有覆盖 Recovery 二进制。
- Map result: 五个候选 way 的 Recovery 世界坐标最大误差 `0.000006565`；新华路与法华镇路 asphalt edge 净距分别为 `4.880222`、`10.667135` 场景单位；候选间最小间距 `0.798917`，与四个相邻未知 way 最小间距 `1.169289`，均无 footprint 相交；当前 start 最小建筑净距 `7.053485`。
- Independent review result: 几何、道路不压线、邻接不碰撞均通过；但 TJAD 照片不能把 6/7/8 号楼和保留体量映射到五个 way，且邻近 `864505170` 是否属于临法华镇路体量仍未知，所以 formal membership 与 map acceptance 均保持 `blocked-evidence`。
- Remaining inference: 6/7/8号楼、保留体量、临法华镇路界面、精确高度、入口罗盘方向。
- Performance impact: 无公共运行时代码变化；只新增本栋证据、候选 Massing 与专项。
- Rollback point: 本建筑独立分支基线 `48c0153`；旧 Hero 和 Recovery/Hold 均保留。
