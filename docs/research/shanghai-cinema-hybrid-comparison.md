...
> `Massing > 60 / Identity 38–60 / Detail ≤ 38` 仅是历史实验分层，不再是生产阈值。
> `docs/research/building-quality-tiers-and-loading-contract.md` 已覆盖旧地图距离 LOD 结论：
> 地图始终展示 Hybrid Identity，Massing 只允许在启动封面遮挡期间存在，
> 不会按地图距离切换；进入本地游览后，Hero 只按玩家到建筑的实际距离加载。
> “进入建筑”只负责快速定位，不是 Hero 开关。上海影城 Hybrid Identity 已按该合同恢复到生产地图。
...
当时的测试结论为 **架构可行，但不应直接把当时的试验版替换到生产场景**。
该判断记录的是 2026-07-22 的实验状态；后续修正并通过生产合同后，上海影城
Hybrid Identity 已于 2026-07-24 恢复到生产地图。
...
| 距离层 | 触发范围 | 加载内容 | 目的 |
| --- | --- | --- | --- |
| Massing | `> 60` 场景单位 | 程序化主体，零 GLB | 实验远景立即出现正确占位和主尺度 |
...
- 优先试迁规则住宅、里弄和街区楼体；
- 保留地标独有轮廓为小 GLB；
- 在生产地图全览固定加载 Identity；本地游览再按实际距离加载 Hero；
- 把本轮“同材质程序化体块也必须实例化/合批”写入正式实现标准。

### No-Go

- 当时的试验版不直接替换生产上海影城；后续生产恢复以 2026-07-25 合同和
  Decision log 为准；
...
- 废止生产地图中的距离阈值以及 Massing / Identity / Full 往返切换；地图阶段固定显示 Identity；
- 恢复本地游览的实时距离采样：标准网络在进入 Hero 近景阈值时请求完整资产，超过退出阈值后恢复 Identity；
- `?start=cinema` 只把人物定位到上海影城附近，因此会间接满足距离条件，但它本身不是质量开关；
- 弱网在同一近景位置长期保留 Identity，不请求 Hero；
