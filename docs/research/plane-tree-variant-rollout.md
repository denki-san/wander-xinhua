...
- 三个街景 GLB 的旧根颈版预算为 `700 KB`；2026-07-23 的开放枝架与小叶簇
  视觉升级把质量预算调整为不超过 `1.4 MB`。
...
- 旧 A/B/C 的七到八个大球冠改为可见主枝、次枝、细枝和沿枝分布的小叶簇；
  每棵保留中央和侧向天空冠隙，不再用一个圆团遮住枝架。
- 三款 GLB 分别为 `310052`、`305140`、`348572` 字节，合计
  `963764` 字节；三角面分别为 `5644`、`5512`、`6004`，仍保持
  `1 node / 1 mesh / 6 materials / 0 images / 0 textures`。
- 运行时缓存版本改用各自 SHA 前 12 位，继续由 `InstancedMesh` 复用；
  Hero 仍只替换一个白名单树位。
- 实际 1440×900 `?start=house315` 页面转向道路纵深后，普通树阵可以读出
  高位分叉、小叶簇、相邻变体和冠隙；证据为
  `test_xinhua_autumn_storybook_v2_plane_tree_street_desktop.png`。
- 控制台应用错误为 `0`；只保留既有 Three.js `Clock` 和
  `PCFSoftShadowMap` deprecated warning。
