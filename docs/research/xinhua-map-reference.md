...
- `docs/research/data/xinhua-boundary-osm-20260716-080509.json`
- `docs/research/data/xinhua-roads-osm-20260716-080509.json`

原始快照只保存经纬度与 OSM 标签，不含场景比例；当前五倍版本从这组最新完整快照离线重放生成，避免因公共 Overpass 节点临时不可用而阻断本地开发。
...
- 基础地理比例为 1 个场景单位对应 13.5 米；当前体验世界统一放大 5 倍，运行比例为 1 个场景单位对应 2.7 米。
...
- OSM 中心线长度：约 147.7 米
- 场景锚点：`[106.6662, 12.9045]`
- 全局旋转：`2.997763` radians
- 水平缩放：`0.581898`
- 垂直缩放：`1.5`

人物尺寸保持不变；幸福里建筑、街具和碰撞与地图空间一起统一放大 5 倍。X/Z 位置、方向、长度和碰撞盒始终使用同一个地图投影与水平缩放。
...
公共接口不可用、但只需调整表现比例时，可从最近一组完整原始快照确定性重放：

```bash
/opt/homebrew/bin/node scripts/test_generate_xinhua_map.mjs --from-raw-snapshot
```

离线重放不会创建或伪造新的抓取文件；运行时元数据会记录实际使用的原始快照名称与模式。
