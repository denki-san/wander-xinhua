...
- 位置证据：地址与设计方资料明确；OSM 无独立条目，按 345 弄入口、既有 365 弄落点和新华路中心线推定为场景坐标 `[-56, 64]`，快照明确标记 `inferred: true`。
- 主要参考：
...
- 轮廓与空间：狭长弄堂型花园，蜿蜒浅色洗石路径贯穿，两侧镜面墙把空间视觉放大。
...
- 位置证据：德必官网地址明确；OSM 缺少 525 号独立条目。先以道路锚点 `[-91, -13]` 建立研究快照，再结合交大法华校区边界和道路北侧建筑群，把资产中心退到 `[-110, -30]`，避免占用法华镇路；两步均标记为推定。
- 主要参考：
...
- 地址：法华镇路与香花桥路交会处。
- 位置证据：澎湃转载上海长宁资料明确为“法华镇路香花桥路口”；地图中两条 OSM 道路中心线交点为场景坐标 `[-70.17, -0.57]`。资产中心沿北侧人行界面退到 `[-75, -7]`，避免牌坊横压车行道。
- 主要参考：
...
- 轮廓与材料：灰石三间纪念牌坊，中间横额、两侧短瓦檐，中央和两侧嵌深褐说明牌。
- 专属识别点：
  1. 中央四柱三间石构；
  2. “法华遗韵”金字横额；
  3. 两侧灰瓦短檐；
  4. 深褐历史说明板；
  5. 柱头简化云纹。
- 碰撞：仅四根石柱和两侧展板阻挡，中门保持可穿行。

## FICS 新华 365

- 地址：新华路 365 弄。
- 建筑事实：东华大学科技园改造项目，前身包含德式花园洋房、上海搪瓷研究所和学生宿舍；园区包含多栋不同年代建筑，2023 年底完成改造。
- 位置证据：政府资料和现有新华公馆落点明确；OSM 缺少园区独立条目。研究中心先按 365 弄和 345 弄路网估为 `[-75, 82]`，集成时采用园区枢轴 `[-74.2, 70.6]`，使完整模型中的新华公馆仍落在原验证坐标 `[-63, 75]`；推定状态不变。
- 主要参考：
...
- 专属识别点：
  ...
  5. 新华公馆红砖拱窗与露台；
  6. 围绕中央广场组织的多年代建筑群。
- 场地环境：开放广场、条带铺装、树阵、露台和园区主入口均可步行。
- 省略：具体商户招牌、临时活动装置和车辆。
...
- 所有照片仅保存在研究目录中，用于人工比对建筑事实，不打包进运行时 GLB，也不进入公开页面。
