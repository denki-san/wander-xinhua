...
| ---: | --- | --- | --- | --- | --- |
| 1 | `shanghai-cinema` | 上海影城 | Hero、Hybrid Identity composite、正式 Massing、MCP 1/2/3 与三档运行时证据均保留；Film Art Center 新 footprint 已消除旧邻栋 AABB 假冲突；项目号 `2102CN0309D01`、新华路160号及“广场平缓衔接人行道”获得官方复核 | `blocked-map` / current assets retained | XHS 四组慢速查询与“双控制点+可量退界”验收合同已备但尚未执行；地址、中心点、单张正面或无比例总平不能授权拖动，实际查询耗尽前不得提前停用 |
...
| 4 | `xinhua-villas-211` | 新华别墅·211弄 | Massing v3 的 MCP1、地图、碰撞和 Three.js 已通过并冻结；官方资料新确认两弄合计29幢，并点名211弄1号、2号、12号，但没有门牌到九个 accepted OSM way 的正式总平 | `blocked-hero-identity` / Massing retained | same-member多视角、门牌到九个accepted way绑定与侧后/roof-back的XHS查询合同已备未执行；旧跨资产Hero继续Hold，实际查询耗尽前不得提前停用 |
| 5 | `xinhua-villas-329` | 新华别墅·329弄 | 四成员 Massing v3、MCP1、正式地图与 Three.js 已通过；官方资料点名17/36/38/40/42等成员语境，并证明329弄地址空间还含43/56/67等不同用途或组团 | `blocked-hero-identity` / Massing retained | compound边界、17/36/38/40/42到OSM way绑定、同成员多视角/尺度/入口/背面/roof-back与Hero lineage的XHS查询合同已备未执行；67号加梯总平和公网泛地址不得冒充绑定，实际查询耗尽前不得提前停用 |
...
| 8 | `shanghai-orchestra` | 上海民族乐团 | Recovery clean-v2 Massing 的 MCP1 与诊断 Three.js 保留；官方资料一致确认新华路336号，设计项目资料支持6/7/8号楼的功能语境 | `blocked-membership` / diagnostic only | 主体楼号/compound边界/入口道路/footprint控制点的XHS查询合同已备未执行；地址、节目单、几何安全或宣传语境仍不能冒充way binding，查询耗尽前不得提前停用 |
...
| 11 | `xinhua-community-center` | 新华·社区营造中心 | way/864493234 Building-4 Massing、MCP1 与诊断 Three.js/碰撞保留；官方资料确认345弄整体更新、营邑规划委托及社区中心启用，但未公开道路总平/横断面 | `blocked-map` / diagnostic only | 双路缘、中心线/偏移、可量尺度、日期、surface/access 的XHS查询合同已备未执行；`4.575771 m` 仍只是零重叠反推上限，禁止任意移楼、缩放或改窄共享道路 |
| 12 | `debi-fahua-525` | 德必法华525 | 三张证据与 Recovery clean-v2 Massing 只读保留；运营方确认同址独立改造园区、6层和 `5428.17㎡`，政府托管服务页另称“一万多㎡”，两者未调和且都不是可配准 footprint | `blocked-membership-map-lineage` / Hold | compound成员/庭院、法华镇路双路缘/宽度/surface/access与入口控制的XHS查询合同已备未执行；真实重叠仍为`0.633229`，禁止移楼、缩放或改窄共享道路 |
| 13 | `fahua-heritage` | 法华遗韵 | U盘既存XHS视频两帧已按原字节接入：连续右前斜向到近正面，补足右侧短瓦檐/侧板纵深、相邻518墙面、铺地、左侧窄通行和路缘高差；旧基线与 Recovery 子阶段继续只读保留 | `blocked-map-rear-scale` / Hold | side/depth 与 street context 已通过；主窗口复算确认518无法与保存OSM闭合，照片到世界控制点仍为0，完整背面、实测尺度、footprint、路缘/步道边界及地图校准继续未知；新外置动态证据快照待本批统一生成，禁止提前建模或升级MCP |
| 14 | `fics-xinhua-365` | FICS新华365 | Recovery Massing 的 MCP1、投影与诊断 Three.js 保留；政府资料确认东华大学关联、365弄园区身份及“封闭园区转开放公共社区”目标 | `blocked-membership-map-tiers` / diagnostic only | 园区成员表/边界、五个OSM候选逐栋绑定、way/864493177与service surface、way/577252268的surface/width/cover/access及365弄入口连续性的XHS合同已备未执行；“开放社区”宣传不能冒充道路或几何，查询耗尽前不得提前停用 |
| 15 | `xingfuli-west` | 幸福里·西区 | Hero 保留；Identity v2 / Massing v2 严格派生；Blender MCP 1/2/3 与单页三档、两级 fallback、性能、碰撞均通过 | `blocked-map` / explicit QA uses lineage v2 / production Hero unchanged | 摄影者再次确认九图是从幸福路入口到对侧道路的连续实拍链；当前消息仍未提供可读取的九个文件对象，动态证据快照为pending。原图物化后先归档U盘，再判断底层通道/洞口、坡度、墙体和幸福路路缘，公网图不得冒充 |
| 16 | `xingfuli-center` | 幸福里·中区 | Hero 保留；Identity v2 / Massing v2 严格派生；Blender MCP 1/2/3、地图、单页三档、两级 fallback、性能、碰撞均通过 | `done` / lineage v2 tiers / production Hero unchanged | 无；九图路线陈述可用于顺序对照，但当前原图仍未物化；只有外置动态证据快照和逐图分区完成后，才能把画面事实加入模型验收 |
| 17 | `xingfuli-east` | 幸福里·东区 | Hero 保留；Identity v2 / Massing v2 严格派生；Blender MCP 1/2/3 与单页三档、两级 fallback、性能、碰撞均通过 | `blocked-map` / explicit QA uses lineage v2 / production Hero unchanged | south entry、entry matrix wall 与 lane base 仍和番禺路面相交；摄影者认为第9张对侧道路“应该是番禺路”，但原图文件和可见路名尚未物化，仍必须以画面+地图双证据确认，禁止把不确定陈述直接升格为地图事实 |
