...
| 3 | `one-step-garden` | 一尺花园 | Evidence Gate、分体 Massing、MCP 1、实际地图门、独立 Hero v2、修复后 MCP 2、独立 Identity v1 与主窗口 MCP 3 三档复核均已通过；首次发白失败证据继续保留，旧误绑/范围污染 Hero 继续只读 Hold | `in-scope` / source `45e52e1` / integrated `e0da1bc` | 接入本建筑 Hero v2 / Identity v1 / Massing tier runtime，并完成三档、fallback、入口与前后间隙的真实 Three.js 终验；不得删除旧 Hero 或加入树木、装饰和场地 |
