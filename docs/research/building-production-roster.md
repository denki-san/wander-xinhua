...
| 13 | `fahua-heritage` | 法华遗韵 | U盘既存XHS视频两帧已按原字节接入：连续右前斜向到近正面，补足右侧短瓦檐/侧板纵深、相邻518墙面、铺地、左侧窄通行和路缘高差；旧基线与 Recovery 子阶段继续只读保留 | `blocked-map-rear-scale` / Hold | side/depth 与 street context 已通过；完整背面、实测尺度、footprint、路缘/步道边界及地图校准仍未知，禁止提前建模或升级MCP；原视频未保存，U盘原manifest路径错配记录保持只读 |
...
| 15 | `xingfuli-west` | 幸福里·西区 | Hero 保留；Identity v2 / Massing v2 严格派生；Blender MCP 1/2/3 与单页三档、两级 fallback、性能、碰撞均通过 | `blocked-map` / explicit QA uses lineage v2 / production Hero unchanged | 用户九张原图当前为 `0 files`；底层通道/洞口、坡度、墙体、幸福路路缘的XHS查询合同已备但尚未执行，公网图不得冒充用户实拍；禁止任意挖洞或提前停用 |
...
| 17 | `xingfuli-east` | 幸福里·东区 | Hero 保留；Identity v2 / Massing v2 严格派生；Blender MCP 1/2/3 与单页三档、两级 fallback、性能、碰撞均通过 | `blocked-map` / explicit QA uses lineage v2 / production Hero unchanged | south entry、entry matrix wall 与 lane base 仍和番禺路面相交；东端连续路线、道路身份、路缘、三处构件的XHS查询合同已备但尚未执行，第9张原图仍为0，禁止公网图替代或任意位移 |
