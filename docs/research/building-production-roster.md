...
| ---: | --- | --- | --- | --- | --- |
| 1 | `shanghai-cinema` | 上海影城 | Hero、Hybrid Identity composite、正式 Massing、MCP 1/2/3 与三档运行时证据均保留；Film Art Center 新 footprint 已消除旧邻栋 AABB 假冲突 | `blocked-map` / current assets retained | 仍缺可地理配准的本楼 footprint/道路退界锚点，Hero generator 也存在 source drift；禁止用道路距离等价位置或旧邻栋冲突记录冒充完成 |
...
| 8 | `shanghai-orchestra` | 上海民族乐团 | Recovery clean-v2 Massing 的 MCP1 与诊断 Three.js 保留；五个 OSM 候选的道路和邻接均可复算 | `blocked-membership` / diagnostic only | 五个匿名 way 均无名称、门牌或 relation，必须先绑定 compound 成员；禁止复用旧 Hero 或从 proximity 猜 Identity |
...
| 11 | `xinhua-community-center` | 新华·社区营造中心 | Building-4 Massing、MCP1 与诊断 Three.js/碰撞保留 | `blocked-map` / diagnostic only | 当前 service road 仍重叠约 `0.402635` scene unit，且本地道路宽度/关系证据耗尽；不得任意位移或缩放 |
...
| 13 | `fahua-heritage` | 法华遗韵 | 旧牌坊与单张正面证据、Recovery 合格子阶段只读保留 | `blocked-evidence-map` / Hold | 缺侧后、入口和街道语境；用户睡眠期间不查 XHS，后续仍找不到时才按授权从游戏停用，文件永久保留 |
