...
| 2 | `film-art-center` | 上海电影艺术中心 | 确定性 Massing、MCP 1、地图门、修复后 Hero MCP 2、独立 Identity 与主窗口 MCP 3 三档复核均已整合；Identity 从冻结 Hero `33daaaf0…` 派生，不采用 recovery generic box | `in-scope` / source `2cdcd56` / integrated `e69bc24` | 接入本建筑 tier runtime 并完成 Hero / Identity / Massing 真实 Three.js 同机位终验；Hero 既有草坪/灌木/庭院灯保持冻结，不修改、不删除、不向低档传播 |
| 3 | `one-step-garden` | 一尺花园 | Evidence Gate、分体 Massing、MCP 1、实际地图门、独立 Hero v2 与主窗口 MCP 2 均已通过；首次 MCP2 因未写 Principled Base Color 发白而被拦截，失败证据与修复后证据均保留；旧误绑/范围污染 Hero 继续只读 Hold | `in-scope` / source `abbdd86` / integrated `bd06d7b` | 从冻结 Hero v2 `026565ba…` 派生独立 Identity，完成 MCP 3 三档与真实 Three.js runtime；不得删除旧 Hero 或向新 tier 加入树木、装饰和场地 |
...
| 5 | `xinhua-villas-329` | 新华别墅·329弄 | 两张官方成员证据与误绑审计已整合至 `2e185c6`；旧 Hero/voxel Massing 实为211弄2号污染基线，clean-v2 是五个未绑定 OSM 盒 | `blocked-evidence` / source `fb652cf` | 缺 compound canonical、入口、成员布局和位置绑定；不得恢复旧 Hero/Massing，先补329弄整体证据 |
| 6 | `house-315` | 新华路315号住宅 | 官方 canonical、俯瞰纵深、入口/门牌证据已闭合；subject-specific Massing、主窗口 MCP 1 与实际 Three.js 地图门已通过；旧 Hero disposition `dc89f9e` 确认主体结构不符、120 个零面积三角面并烘焙庭院/围栏/灯/花箱/铺装，冻结为只读 Hold | `in-scope` / Massing complete / Hero v2 authorized | 保持 1930/1949 年代冲突为 Unknown；从已验收 Massing 独立重建无树木装饰场地的 `hero-v2`，通过主窗口 MCP 2 前不得制作 Identity 或接公共 runtime |
