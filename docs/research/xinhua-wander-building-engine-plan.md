...
- Status: `production-integration-candidate-local`
...
- Phase-one formula: 一个 Archetype、两栋建筑、一个 CLI、三个审核门（两栋只计正式生产案例）
...
- Explicitly excluded: 后台、数据库、Worker、任务队列、Meshy、push、部署与远端发布；
  只允许在独立集成 worktree 本地合并

## 1. 本轮结论

第一阶段不再以“五栋、三个 archetype、完整三档 Runtime Package”为起点。本轮只验证
一个更小、但能证伪核心假设的 Spike。当前生产集成口径固定为：

> `house-315` 与 `hudec-memorial` 是 2 栋正式生产案例；
> `sun-ke-villa` 保留全部 DSL、生成资产、测试和记录，但只算 1 栋实验案例。

历史 Spike 曾用 House 315 与孙科别墅验证通用 Compiler；该历史事实继续保留，但
不能再把孙科别墅计入本阶段“两栋正式生产建筑”的交付数字。
...
| --- | --- | --- | --- |
| `house-315` | 正式生产案例 1、回归基线 | 官方正面、俯瞰、入口、门牌和身份细节已闭合；用于证明生产 Compiler 回归 | `production` |
| `hudec-memorial` | 正式生产案例 2、默认 promotion | 用户补充照片覆盖正面、侧向、历史正面和烟囱校正；A 方案已进入本地默认 registry，且保留原子回滚 | `production` |
| `sun-ke-villa` | 保留的实验案例 | 历史盲测和三层验收完整，继续用于 Compiler 回归；不进入本轮默认 Building Engine 生产 promotion/manifest | `experimental-retained` |

“一个 Archetype、两栋建筑”只计前两行。孙科别墅的 DSL、GLB、`.blend`、碰撞、
Sandbox、审核记录和自动测试不得删除，但它的 2 个 artifact QA 只报告为实验回归。
...
若后续发现 `sun-ke-villa` 的现有证据、来源权利或主体绑定不满足 Evidence Gate，
必须把实验案例改为 `blocked-evidence`，不得临时换用生成式补全或在 Python 中
硬编码伪事实，也不得因此把它提升进正式生产口径。
...
### 3.4 `hudec-memorial` 覆盖（正式生产案例）

详细的用户图片、方向标注、A/B/C 拓扑比较、A 方案选择和烟囱高度校正记录见
`docs/research/hudec-memorial-building-engine-blind-test-brief.md`。正式覆盖至少包括：

- 正面稍偏斜的现代照片；
- 贴近烟囱与长坡玻璃翼的侧向广角照片；
- 完全正面的历史照片；
- 用户对双烟囱所在屋坡以及白色、红色部分加高的直接标注。

缺失背面和测绘尺寸仍标记为未知，不用照片外推补齐。
...
```text
inspect
validate --asset <id|all> [--scope <production|experimental|all>]
build --asset <id|all> --stage <massing|master|all> [--scope <production|experimental|all>]
review --asset <id> --gate <evidence|massing|final> --decision <decision>
qa --asset <id|all> [--scope <production|experimental|all>] [--sandbox-origin <origin>]
status --asset <id|all> [--scope <production|experimental|all>]
```
...
- `--scope production` 顺序处理两栋正式案例；
- `--scope experimental` 只处理保留的孙科实验案例；
- 未写 `--scope` 的 `all` 表示全部三个 Case，不得把结果误报为三栋生产交付；
- 所有批量构建都不并发运行 Blender；
...
它只认识 DSL 类型与 Art Profile，不认识 `house-315`、`hudec-memorial` 或
`sun-ke-villa` 业务分支。
...
└── cases/
    ...
    │   ├── building-dsl.json
    │   └── reviews/
    └── sun-ke-villa/  # experimental-retained
        ├── building-case.json
        ├── building-dsl.json
        └── reviews/
```
...
- `house-315` 与 `hudec-memorial` 两栋正式生产案例的 Evidence Gate 均通过；
- 两栋正式生产案例使用同一个 DSL schema、Art Profile 和 Python Compiler；
- Compiler 不包含任何按资产 ID 分支；
- `sun-ke-villa` 的既有 DSL、生成资产、审核和测试继续保留，但只标记为
  `experimental`；
- CLI 是唯一 Runner，支持单资产；
- 两栋正式生产案例的 Massing 均有当前 SHA 对应的 Gate M 记录；
- 两栋正式生产案例的 Low-poly Master、`.blend`、GLB 和固定机位产物齐全；
- GLB、预算、lineage 与碰撞自动检查通过；
- 两栋正式生产案例在真实 Three.js Sandbox 中可见、接地、方向正确；
- Final Gate 有当前 SHA 对应的手工结论；
- 新动态证据进入新的不可变外置快照并全量 SHA 通过；
- 专项测试、`npm test` 和 `npm run lint` 通过；
- 只有 `hudec-memorial` 的审核后 A 方案可进入本地默认 production registry；
  `sun-ke-villa` 的 Building Engine 实验 GLB 不得进入正式 registry 或 promotion；
- 未继续 Meshy QA；
- 未建设后台、数据库、Worker 或任务队列；
- 只在独立集成 worktree 完成本地合并；未 push 或部署。
...
## 14. 历史 Spike 执行结果（House 315 + 孙科别墅）

本节保留 2026-07-28 历史 Spike 的原始口径，不作为当前生产交付计数。历史三个问题
的答案均为“是”，但当时结论只覆盖本地 Spike，不代表生产资产替换或发布：
...
当前集成不删除历史实验结果，但统一改用以下报告：

| 范围 | 资产 | Artifact QA | 默认生产接入 |
| --- | --- | --- | --- |
| 正式生产案例 | `house-315`、`hudec-memorial` | `4 / 4` | House 315 作为生产回归；Hudec A 方案为本地默认 promotion |
| 保留的实验案例 | `sun-ke-villa` | `2 / 2` | 不进入本轮 Building Engine 正式 registry、promotion 或生产 manifest |

正式门执行：

```bash
npm run building:spike -- qa --asset all --scope production --stage all
```

实验回归单独执行：

```bash
npm run building:spike -- qa --asset all --scope experimental --stage all
```

`public/models/building-engine-spike/manifest.json` 只服务隔离
`/building-engine-sandbox`，必须标记为 `isolated-building-engine-sandbox`；
它不是生产 registry。正式 promotion 目录只包含审核后的
`building-engine/promotions/hudec-memorial.json`，不得新增孙科别墅 promotion。
