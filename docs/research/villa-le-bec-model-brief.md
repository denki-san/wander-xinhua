...
六张候选证据已于 2026-07-26 逐张复核：canonical 主建筑图与官方花园概览可用于外部建模；食物、酒室、肖像和室内餐厅四张不能承担外立面或地图绑定。仓库既有 `villa-le-bec-real.jpg` 可补充主立面偏角与当前商业入口细节，但仍没有同楼侧后视角，也不能将五个 OSM footprint 分别绑定为主住宅、原车库与入口。
...
- [x] Recovery Massing 在真实页面中可见，继承 `runtimeGate=pass`；这不等于正式 map pass
- [ ] 正式 Blender MCP1：`pending-main-window-batch`，本窗口没有执行或重复执行
- [ ] Massing 正式地图门：被主住宅 / 原车库 / 入口 footprint 角色未绑定阻塞
