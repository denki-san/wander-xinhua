...
const productionAssetIds = ["house-315", "hudec-memorial"];
const experimentalAssetIds = ["sun-ke-villa"];
const assetIds = [...productionAssetIds, ...experimentalAssetIds].sort();
...
test("Building Engine 只声明 garden-villa，并区分两栋正式案例与一个实验案例", () => {
  ...
  for (const assetId of assetIds) {
    ...
    assert.equal(
      buildingCase.deliveryRole,
      productionAssetIds.includes(assetId) ? "production" : "experimental",
    );
  ...
  assert.match(
    productionizationPlan,
    /2 栋正式生产案例.*1 栋保留的实验案例/,
  );
});
...
test("Compiler 保持数据驱动，单一 CLI 覆盖最小 Pipeline", () => {
  ...
  const qaResults = JSON.parse(qa.stdout);
  assert.equal(qaResults.length, assetIds.length * 2);
  assert.equal(
    qaResults.filter(({ artifact }) => artifact.deliveryRole === "production").length,
    productionAssetIds.length * 2,
  );
  assert.equal(
    qaResults.filter(({ artifact }) => artifact.deliveryRole === "experimental").length,
    experimentalAssetIds.length * 2,
  );

  const scopedStatus = spawnSync(
    process.execPath,
    [
      "scripts/building_engine_spike.mjs",
      "status",
      "--asset",
      "all",
      "--scope",
  ...
  assert.deepEqual(
    JSON.parse(scopedStatus.stdout).map(({ assetId }) => assetId),
    productionAssetIds,
  );
});
...
test("Sandbox 仍保持隔离，只有已审核的 Hudec promotion 接入正式 registry", () => {
  ...
  const sandboxManifest = readJson(
    "public/models/building-engine-spike/manifest.json",
  );
  ...
  assert.equal(
    sandboxManifest.manifestRole,
    "isolated-building-engine-sandbox",
  );
  assert.deepEqual(sandboxManifest.productionAssets, productionAssetIds);
  assert.deepEqual(sandboxManifest.experimentalAssets, experimentalAssetIds);
  assert.equal(
    sandboxManifest.assets["sun-ke-villa"].deliveryRole,
    "experimental",
  );
  assert.deepEqual(
  ...
  assert.ok(!promotedLandmarks.some(({ id }) => id === "sun-ke-villa"));
