...
test("幸福里中栋统一18栋状态已由新 lineage 与运行时记录闭合", async () => {
  const status = await json("docs/research/exact-18-building-status.json");
  const center = status.buildings.find(({ id }) => id === "xingfuli-center");
  assert.equal(center.hero, "pass-retained-strict-parent");
  assert.equal(center.identity, "pass-lineage-v2");
  assert.equal(center.massing, "pass-lineage-v2");
  assert.equal(center.mcp3, "pass-lineage-v2");
  assert.equal(center.map, "pass");
  assert.equal(center.threeJs, "pass-lineage-v2-three-tier-fallback-performance-collision");
  assert.equal(center.overall, "complete");
  assert.equal(center.records.includes(recordPath), true);
  assert.equal(status.summary.completeIds.includes("xingfuli-center"), true);
});
