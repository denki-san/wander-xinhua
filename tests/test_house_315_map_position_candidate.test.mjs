...
test("House315 地图位置候选保留建筑分支边界记录，三档二进制继续 Hold", async () => {
  ...
  for (const [path, historicalSha] of Object.entries(runtimeQa.sharedBaseline.files)) {
    assert.match(path, /^(app\/scene\/|app\/xinhua-experience\.tsx)/);
    assert.match(historicalSha, /^[0-9a-f]{64}$/);
  }
