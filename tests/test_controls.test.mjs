...
test("满幅 180° 转向在目标手感窗口内达到九成", () => {
  ...
  while (current.angleTo(target) > Math.PI / 10 && elapsed < 1) {
    dampTangentTowards(current, target, up, 9, frame, current, 12);
    elapsed += frame;
  }

  assert.ok(elapsed >= 0.2, `转向过快：${elapsed.toFixed(3)}s`);
...
test("进入游玩态时立即切到低位后肩镜头且视觉合成器不重挂", async () => {
  ...
  assert.match(world, /const CHARACTER_MAX_TURN_SPEED = 12/);
