...
test("双尺度视图让全览镜头跟随人物并在闲逛态放大环境而非人物", async () => {
  ...
  assert.match(world, /showDetailLabels=\{exploring\}/);
  assert.match(world, /showDetailModels && \(\s*<>/s);
  assert.match(world, /<Suspense fallback=\{null\}>\s*<ShangshengXinsuoBlock \/>\s*<\/Suspense>/s);
  assert.match(world, /<Suspense fallback=\{null\}>\s*<XinhuaRoadPlaneTrees \/>\s*<\/Suspense>/s);
  assert.match(
    world,
    /<Suspense fallback=\{null\}>\s*<XinhuaRoadLandmarks showLabels=\{showDetailLabels\} \/>\s*<\/Suspense>/s,
  );
