...
test("双尺度视图让全览镜头跟随人物并在闲逛态放大环境而非人物", async () => {
  ...
  assert.match(roadLandmarks, /xinhuaRoadDetailHeroId\(\{ loadMode, priorityPreset \}\)/);
  assert.match(roadLandmarks, /const shouldMountModel = mountedModelIds\.has\(landmark\.id\)/);
  assert.match(roadLandmarks, /function useDetailHeroLandmarkIds/);
  assert.doesNotMatch(
    roadLandmarks,
    /landmarkMatchesPreset|LANDMARK_DISTANCE_SAMPLE_SECONDS|LANDMARK_FULL_ENTER/,
  );
  assert.match(
    roadLandmarks,
