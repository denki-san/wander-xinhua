...
test("自然比例人物与五倍环境为幸福里街巷留下充足通行宽度", async () => {
  ...
  assert.match(world, /result\.position\.copy\(SHADOW_CENTER\)/);
  assert.match(world, /exploring \? camera\.position\.x : SHADOW_CENTER\.x/);
  assert.match(world, /target\.position\.copy\(focus\.current\)/);
  assert.match(world, /target=\{target\}/);
...
test("地下和覆盖道路保留在数据中但不会进入地表道路过滤条件", async () => {
  ...
  assert.match(surfaceContract, /return !road\.tunnel && road\.layer >= 0/);
  assert.match(mapSource, /isSurfaceRoad\(candidate\)/);
});
