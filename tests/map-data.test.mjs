...
test("人物尺寸不变且五倍环境为幸福里街巷留下充足通行宽度", async () => {
  ...
  assert.match(world, /const CAMERA_DISTANCE = 6\.4/);
  assert.match(world, /const CAMERA_HEIGHT = 2\.65/);
  assert.match(world, /const CAMERA_TARGET_HEIGHT = 1\.65/);
  assert.match(world, /const CAMERA_SHOULDER_OFFSET = 0\.62/);
