...
test("当前资产口径保留真实数量与最新街具", async () => {
  ...
  assert.match(data, /幸福里入口尖顶路桩/);
  assert.match(data, /幸福里临水圆形石座/);
  assert.match(data, /幸福里临水长形石座/);
  assert.match(data, /幸福里临水窄型花槽/);
  assert.match(data, /sun-ke-villa-identity\.glb\?v=6b541e8ffab4/);
  assert.match(data, /sun-ke-villa-massing\.glb\?v=f233f9defd21/);
