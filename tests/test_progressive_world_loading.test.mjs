...
test("生产主世界让全部建筑遵守 Massing、Identity、Hero 三层和两场景合同", async () => {
  ...
  assert.match(world, /focusPosition: progressiveFocus,\s+center: XINGFULI_POSITION,\s+fullEnterDistance: 72,\s+fullExitDistance: 88/);
  assert.match(world, /focusPosition: progressiveFocus,\s+center: SHANGSHENG_XINSUO_POSITION,\s+fullEnterDistance: 92,\s+fullExitDistance: 112/);
  assert.match(world, /focusPosition: progressiveFocus,\s+center: HUASHAN_GREEN_POSITION,\s+fullEnterDistance: 76,\s+fullExitDistance: 94/);
  ...
  assert.match(roadFull, /LANDMARK_DISTANCE_SAMPLE_SECONDS = 0\.2/);
