...
test("上海影城生产 Identity 复用完整 Hybrid 组合且保持 Hero 原尺度", async () => {
  const [productionIdentity, roadMassing, roadLandmarks, buildRecord] = await Promise.all([
    readFile(
      new URL("app/scene/shanghai-cinema-hybrid-identity.tsx", root),
      "utf8",
    ),
    ...
    readFile(
      new URL("docs/research/build-records/shanghai-cinema-hybrid-identity.json", root),
      "utf8",
    ).then(JSON.parse),
  ]);
  assert.match(productionIdentity, /ShanghaiCinemaProgrammaticBody/);
  assert.match(productionIdentity, /ShanghaiCinemaIdentityGlb/);
  assert.match(productionIdentity, /ShanghaiCinemaRepeatedDetails/);
  assert.match(productionIdentity, /lodStrategy: "custom-landmark-hybrid"/);
  assert.match(productionIdentity, /massingVisibility: "cover-only"/);
  assert.match(productionIdentity, /Array\.from\(\{ length: 32 \}/);
  assert.match(productionIdentity, /Array\.from\(\{ length: 9 \}/);
  assert.match(
    roadMassing,
    /identity && landmark\.id === "shanghai-cinema"[\s\S]*?<ShanghaiCinemaHybridIdentity \/>/,
  );
  ...
  assert.equal(buildRecord.validation.mapStandard.heroRequested, false);
  assert.equal(buildRecord.validation.localNearStandard.heroRequested, true);
  assert.equal(buildRecord.validation.localNearWeak.heroRequested, false);
});
