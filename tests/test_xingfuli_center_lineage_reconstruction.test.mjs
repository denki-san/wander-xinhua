...
const recordPath = "docs/research/xingfuli-center-lineage-reconstruction-2026-07-26.json";
...
test("中心栋 reconstruction record 锁定当前三档与共同来源快照", async () => {
...
test("中心栋历史记录证明构建顺序相反，不能回填 Hero 派生关系", async () => {
  const record = await readJson(recordPath);
...
test("中心栋生成器是并列 stage recipe，不消费 Hero 或 Identity 父级", async () => {
  const record = await readJson(recordPath);
...
test("中心栋 GLB 只携带 stage 信息，不携带严格父级 lineage", async () => {
  const record = await readJson(recordPath);
  ...
  for (const [tierName, artifact] of Object.entries(record.artifacts)) {
    ...
    assert.equal(extras.segment, "center");
...
test("中心栋 deterministic 输出与最小修复边界没有被夸大", async () => {
  const record = await readJson(recordPath);
