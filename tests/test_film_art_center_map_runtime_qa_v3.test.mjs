...
test("Film Art Center v2 终审以真实共同 provenance 关闭旧 blocker", async () => {
  const [acceptance, lineage, historical, status] = await Promise.all([
    readJson(
      "docs/research/film-art-center-final-acceptance-v2-2026-07-26.json",
    ),
    readJson("docs/research/film-art-center-lineage-adjudication-v2.json"),
  ...
  const film = status.buildings.find(({ id }) => id === "film-art-center");
  assert.equal(film.overall, "complete");
  assert.equal(film.map.startsWith("pass-"), true);
  assert.equal(status.summary.complete, 8);
  assert.equal(status.summary.completeIds.includes("film-art-center"), true);
});
