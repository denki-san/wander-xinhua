...
test("道路地标在地图常驻 Identity 并只在进入对应地点时挂载 Hero", () => {
  assert.match(sceneSource, /function useDetailHeroLandmarkIds/);
  assert.match(sceneSource, /xinhuaRoadDetailHeroId\(\{ loadMode, priorityPreset \}\)/);
  assert.match(sceneSource, /heroId \? new Set\(\[heroId\]\) : new Set\(\)/);
  assert.match(sceneSource, /const shouldMountModel = mountedModelIds\.has\(landmark\.id\)/);
  assert.doesNotMatch(sceneSource, /landmarkMatchesPreset/);
  assert.doesNotMatch(
    sceneSource,
    /LANDMARK_FULL_ENTER|LANDMARK_DISTANCE_SAMPLE|distance <= threshold|LANDMARK_STAGGER_INTERVAL_MS/,
  );
});
