...
const DELIVERY_ROLES = new Set(["production", "experimental"]);
...
function caseDeliveryRole(assetId) {
  const role = readJson(join(CASES_ROOT, assetId, "building-case.json")).deliveryRole;
  if (!DELIVERY_ROLES.has(role)) fail(`${assetId} 缺少有效 deliveryRole`);
  return role;
}


function availableAssets(scope = "all") {
  if (scope !== "all" && !DELIVERY_ROLES.has(scope)) {
    fail(`无效 --scope：${scope}`);
  }
  const assets = readdirSync(CASES_ROOT, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name)
    .sort();
  return scope === "all"
    ? assets
    : assets.filter((assetId) => caseDeliveryRole(assetId) === scope);
}


function selectedAssets(value, scope = "all") {
  const allAssets = availableAssets();
  const assets = availableAssets(scope);
  if (!value) fail("缺少 --asset <id|all>");
  if (value === "all") return assets;
  if (!allAssets.includes(value)) fail(`未知资产：${value}`);
  if (!assets.includes(value)) fail(`${value} 不属于 scope=${scope}`);
  return [value];
}
...
function refreshRuntimeManifest() {
  ...
  for (const assetId of availableAssets()) {
    ...
    assets[assetId] = {
      ...
      archetype: dsl.archetype,
      deliveryRole: caseData.deliveryRole,
  ...
  writeJson(path, {
    ...
    manifestRole: "isolated-building-engine-sandbox",
    productionAssets: availableAssets("production"),
    experimentalAssets: availableAssets("experimental"),
    assets,
  });
}
...
function qaArtifact(assetId, stage) {
  ...
  return {
    assetId,
    deliveryRole: caseDeliveryRole(assetId),
    stage,
    status: "pass",
...
function inspectEnvironment() {
  ...
  return {
    ...
    deliveryScopes: {
      production: availableAssets("production"),
      experimental: availableAssets("experimental"),
    },
    archetype: "garden-villa",
    blender: {
      binary: blender,
...
function usage() {
  ...
  node scripts/building_engine_spike.mjs validate --asset <id|all> [--scope <production|experimental|all>]
  node scripts/building_engine_spike.mjs build --asset <id|all> --stage <massing|master|all> [--scope <production|experimental|all>]
  node scripts/building_engine_spike.mjs review --asset <id> --gate <evidence|massing|final> --decision <decision> --reviewer <name>
  node scripts/building_engine_spike.mjs qa --asset <id|all> [--stage <massing|master|all>] [--scope <production|experimental|all>] [--sandbox-origin <origin>]
  node scripts/building_engine_spike.mjs status --asset <id|all> [--scope <production|experimental|all>]`;
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (command === "inspect") {
    ...
    result = selectedAssets(options.asset, options.scope ?? "all")
      .map((assetId) => validateAsset(assetId));
  } else if (command === "build") {
    const stage = options.stage;
    if (!["massing", "master", "all"].includes(stage)) fail("build 需要有效 --stage");
    const outputs = [];
    for (const assetId of selectedAssets(options.asset, options.scope ?? "all")) {
    ...
    result = selectedAssets(options.asset, options.scope ?? "all").map(statusFor);
  } else {
    fail(`未知命令：${command}`);
  }
