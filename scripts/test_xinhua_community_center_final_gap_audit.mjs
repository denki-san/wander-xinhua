import assert from "node:assert/strict";
import { createHash } from "node:crypto";
import { readFile } from "node:fs/promises";
import path from "node:path";
import { fileURLToPath } from "node:url";

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

async function bytes(relativePath) {
  return readFile(path.join(root, relativePath));
}

async function json(relativePath) {
  return JSON.parse(await readFile(path.join(root, relativePath), "utf8"));
}

async function sha256(relativePath) {
  return createHash("sha256").update(await bytes(relativePath)).digest("hex");
}

function parseGlb(buffer) {
  assert.equal(buffer.toString("utf8", 0, 4), "glTF");
  assert.equal(buffer.readUInt32LE(4), 2);
  const jsonLength = buffer.readUInt32LE(12);
  return JSON.parse(buffer.toString("utf8", 20, 20 + jsonLength).trim());
}

function triangleCount(glb) {
  return (glb.meshes ?? []).reduce(
    (sum, mesh) => sum + mesh.primitives.reduce((meshSum, primitive) => {
      const accessor = glb.accessors[primitive.indices ?? primitive.attributes.POSITION];
      return meshSum + accessor.count / 3;
    }, 0),
    0,
  );
}

export async function auditXinhuaCommunityCenterFinalGap() {
  const [audit, manifest, mcp, mapGate, runtime, record, heroGenerator] =
    await Promise.all([
      json("docs/research/xinhua-community-center-final-gap-audit.json"),
      json("docs/research/xinhua-community-center-reference-manifest.json"),
      json("docs/research/xinhua-community-center-blender-mcp-gates.json"),
      json("docs/research/xinhua-community-center-massing-map-gate.json"),
      json("docs/research/xinhua-community-center-threejs-runtime-qa.json"),
      json(
        "docs/research/build-records/tiers/xinhua-road/massing-v2/xinhua-community-center-massing.json",
      ),
      readFile(path.join(root, "scripts/create_requested_poi_models.py"), "utf8"),
    ]);

  assert.equal(audit.assetId, "xinhua-community-center");
  assert.equal(
    audit.status,
    "massing-mcp1-collision-pass-road-hero-identity-blocked",
  );
  assert.equal(audit.scope.browserOpened, false);
  assert.equal(audit.scope.blenderRerun, false);
  assert.equal(audit.scope.sharedFilesModified, false);
  assert.equal(manifest.assetId, audit.assetId);
  assert.equal(manifest.viewCoverageMatrix.sideDepth.status, "unknown");
  assert.equal(manifest.evidenceGate.massingAuthorized, true);
  assert.equal(manifest.evidenceGate.heroAuthorized, false);
  assert.equal(manifest.evidenceGate.identityAuthorized, false);
  for (const reference of audit.evidence.references) {
    assert.equal(await sha256(reference.path), reference.sha256);
  }

  const massingBuffer = await bytes(audit.massing.glb.path);
  const massing = parseGlb(massingBuffer);
  assert.equal(await sha256(audit.massing.generator.path), audit.massing.generator.sha256);
  assert.equal(await sha256(audit.massing.blend.path), audit.massing.blend.sha256);
  assert.equal(await sha256(audit.massing.glb.path), audit.massing.glb.sha256);
  assert.equal(massingBuffer.length, audit.massing.glb.bytes);
  assert.equal(massing.nodes.length, audit.massing.glb.nodes);
  assert.equal(massing.meshes.length, audit.massing.glb.meshes);
  assert.equal(massing.materials.length, audit.massing.glb.materials);
  assert.equal(massing.images?.length ?? 0, 0);
  assert.equal(triangleCount(massing), audit.massing.glb.triangles);
  assert.equal(record.glb.sha256, audit.massing.glb.sha256);
  assert.equal(mcp.source.glbSha256, audit.massing.glb.sha256);
  assert.equal(mcp.mcp1.status, "pass");

  assert.equal(mapGate.mapCalibrationCandidate.namedPoiNode, audit.evidence.binding.namedPoiNode);
  assert.equal(mapGate.mapCalibrationCandidate.buildingWay, audit.evidence.binding.buildingWay);
  assert.equal(mapGate.mapCalibrationCandidate.frontAccessRoadWay, audit.evidence.binding.accessRoadWay);
  assert.equal(
    mapGate.mapCalibrationCandidate.closestNeighborGapSceneUnits,
    audit.map.neighbor.gapSceneUnits,
  );
  assert.equal(mapGate.mapCalibrationCandidate.osmFootprintOverlapCount, 0);
  assert.equal(
    mapGate.runtimeRoadSurfaceDecision.asphaltEdgeClearanceSceneUnits,
    audit.map.road.asphaltEdgeClearance,
  );
  assert.ok(audit.map.road.asphaltEdgeClearance < 0);
  assert.equal(mapGate.gates.formalMapAcceptance, "blocked-road-surface-overlap");

  assert.equal(runtime.asset.sha256, audit.massing.glb.sha256);
  assert.equal(runtime.gates.resourceAndConsole, "pass");
  assert.equal(runtime.gates.collision, "pass");
  assert.equal(
    runtime.collisionReplay.result,
    "pass-wall-stop-with-lateral-slide-no-penetration",
  );
  assert.equal(runtime.gates.formalMap, "blocked-road-surface-overlap");
  assert.equal(runtime.completionBoundary.productionRegistryPromotionAuthorized, false);

  const heroBuffer = await bytes(audit.legacyHero.glb.path);
  const hero = parseGlb(heroBuffer);
  assert.equal(await sha256(audit.legacyHero.glb.path), audit.legacyHero.glb.sha256);
  assert.equal(await sha256(audit.legacyHero.blend.path), audit.legacyHero.blend.sha256);
  assert.equal(
    await sha256(audit.legacyHero.generator.path),
    audit.legacyHero.generator.sha256,
  );
  assert.equal(heroBuffer.length, audit.legacyHero.glb.bytes);
  assert.equal(hero.nodes.length, audit.legacyHero.glb.nodes);
  assert.equal(hero.meshes.length, audit.legacyHero.glb.meshes);
  assert.equal(hero.materials.length, audit.legacyHero.glb.materials);
  assert.equal(hero.images?.length ?? 0, 0);

  const functionStart = heroGenerator.indexOf("def build_xinhua_community_center()");
  const functionEnd = heroGenerator.indexOf("\ndef ", functionStart + 1);
  const heroFunction = heroGenerator.slice(functionStart, functionEnd);
  for (const marker of [
    "community-lawn",
    "community-forecourt",
    "community-main",
    "community-planter-left",
    "community-sports-court",
    "community-toy-house",
    "community-toy-shelf-",
  ]) {
    assert.match(heroFunction, new RegExp(marker));
  }

  assert.equal(audit.strictLineage.formalMapPassed, false);
  assert.equal(audit.strictLineage.heroDerivedFromCurrentMassing, false);
  assert.equal(audit.strictLineage.heroMatchesBuilding4OnlyBoundary, false);
  assert.equal(audit.strictLineage.identityDerivationAuthorized, false);
  assert.equal(audit.legacyHero.status, "hold-not-mcp2-candidate");
  assert.equal(audit.gates.mcp2Hero, "not-entered");
  assert.equal(audit.identity.status, "not-created");
  assert.equal(audit.gates.mcp3, "not-entered");

  return {
    assetId: audit.assetId,
    status: audit.status,
    evidence: audit.gates.building4EvidenceForMassing,
    osmBinding: audit.gates.osmBindingForMassing,
    mcp1: audit.gates.mcp1Massing,
    collision: audit.gates.threeCollision,
    map: audit.gates.formalMapAcceptance,
    hero: audit.gates.heroCandidate,
    mcp2: audit.gates.mcp2Hero,
    identity: audit.gates.identity,
    buildingComplete: audit.gates.buildingComplete,
  };
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  process.stdout.write(
    `${JSON.stringify(await auditXinhuaCommunityCenterFinalGap(), null, 2)}\n`,
  );
}
