"""从已验收父档真实删减幸福里中栋 Identity v2 / Massing v2。
...
EXPECTED_PARENT_NAMES = {
    "identity-v2": {
        "glb": "xingfuli-center.glb",
        "blend": "xingfuli-center.blend",
        "parentTier": "hero",
        "expectedObjects": 115,
    },
    "massing-v2": {
        "glb": "xingfuli-center-identity-v2.glb",
        "blend": "xingfuli-center-identity-v2.blend",
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="从父级资产真实删减幸福里中栋 lineage v2 候选",
    )
...
def validate_inputs(args: argparse.Namespace) -> dict[str, str]:
    ...
    committed_generator_sha = git_blob_sha256(
        args.source_commit,
        "scripts/create_xingfuli_center_lineage_v2_models.py",
    )
...
def identity_removal_reason(name: str) -> str | None:
    if name == "center-lane-base":
        return "exclude-site-lane"
    if name.startswith("center-pool-") or name.startswith("center-bridge-"):
        return "exclude-site-hardscape"
    if "-material-" in name:
        return "remove-hero-material-detail"
    if "-balcony-post-" in name:
        return "remove-hero-balcony-post-detail"
    if "-bay-edge-fin-" in name:
        return "remove-hero-bay-fin-detail"
    return None


def massing_removal_reason(name: str, layout: dict) -> str | None:
    if "-sill-" in name:
        return "remove-identity-sill"
    if "-balcony-rail-" in name:
        return "remove-identity-balcony-rail"

    match = re.fullmatch(r"(.+)-window-(\d+)-(\d+)", name)
    if match:
        building_id, _floor, column = match.groups()
        building = next(
            item for item in layout["buildings"] if item["id"] == building_id
        )
        massing_columns = max(3, int(building["width"] / 4.1))
        if int(column) >= massing_columns:
...
def rerender_clear_street_view(slug: str, preview_dir: Path) -> None:
    ...
    bpy.context.scene.render.filepath = str(
        preview_dir / f"test_{slug}_street_preview.png"
    )
...
def derive_child(args: argparse.Namespace, lineage: dict[str, str]) -> dict:
    ...
    for obj in list(bpy.context.scene.objects):
        ...
        reason = (
            ...
            else massing_removal_reason(obj.name, layout)
        )
    ...
    tier_dir = args.tier
    slug = f"xingfuli-center-{tier_dir}"
    source_dir = SOURCE_BASE / tier_dir
    output_dir = OUTPUT_BASE / tier_dir
    preview_dir = PREVIEW_BASE / tier_dir / "xingfuli-center" / "headless"
    source_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    preview_dir.mkdir(parents=True, exist_ok=True)

    scene = bpy.context.scene
    scene["asset"] = "xingfuli-center"
    ...
    rerender_clear_street_view(slug, preview_dir)
    legacy.merge_for_export(slug, len(retained), args.tier, "center")
    merged = legacy.ASSET_OBJECTS[0]
    merged["asset"] = "xingfuli-center"
...
def main() -> None:
    ...
    print("XINGFULI_CENTER_LINEAGE_V2=" + json.dumps(result, ensure_ascii=False))
