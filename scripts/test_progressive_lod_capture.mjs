...
const CDP_HTTP = process.env.TEST_CDP_HTTP ?? "http://127.0.0.1:9225";
...
const CASES = [
  {
    ...
    mode: "hybrid",
    distance: "far",
    file: "test_shanghai-cinema_progressive_far-massing.png",
  },
  {
    id: "medium-identity",
    mode: "hybrid",
    distance: "medium",
    file: "test_shanghai-cinema_progressive_medium-identity.png",
  },
  {
    id: "near-full",
    mode: "baseline",
    distance: "near",
    file: "test_shanghai-cinema_progressive_near-full.png",
  },
];
...
async function waitUntilReady(client, timeoutMs = 30000) {
  ...
  throw new Error("等待渐进式 LOD 样片加载超时");
}

async function captureCase(client, testCase) {
  const url = `${BASE_URL}/hybrid-model-test?mode=${testCase.mode}`
    + `&distance=${testCase.distance}&view=front`;
...
async function main() {
  ...
  client.close();
  const output = { viewport: VIEWPORT, results };
  await writeFile(
    path.join(OUTPUT_DIR, "test_shanghai-cinema_progressive_lod_metrics.json"),
