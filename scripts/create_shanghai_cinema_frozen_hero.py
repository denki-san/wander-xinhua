"""从历史共享生成器快照隔离复现上海影城 Hero。

上游共享快照来自 commit 869e92d83ea177cd4babe73d61ed5657e8f15ddd，
原始文件 SHA-256 为 6ea5fc19f98f6339d83063bafea9c0edd66ca07d2bb171be08f61d63fed3488d。
本文件只改隔离输出目录与单资产入口，不覆盖 public Hero 或共享生成器。
"""
...
OUTPUT_DIR = ROOT / "test_artifacts/shanghai-cinema-hero-reproduction/run"
SOURCE_DIR = ROOT / "test_artifacts/shanghai-cinema-hero-reproduction/run"
PREVIEW_DIR = ROOT / "test_artifacts/shanghai-cinema-hero-reproduction/previews"
FROZEN_SHARED_GENERATOR_COMMIT = "869e92d83ea177cd4babe73d61ed5657e8f15ddd"
FROZEN_SHARED_GENERATOR_SHA256 = (
...
def main() -> None:
    ...
    if requested and requested != {"shanghai-cinema"}:
        raise ValueError("本冻结生成器只允许 --asset=shanghai-cinema")
    builders = [
        (slug, builder)
